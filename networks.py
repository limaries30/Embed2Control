...
class Transition(nn.Module):
    def __init__(self, net, z_dim, u_dim, device='cuda:0'):
    ...
    def plan(self, z_bar_t, u_t):
        ...
        mean = A_t.bmm(z_bar_t.unsqueeze(-1)).squeeze(-1) + B_t.bmm(u_t.unsqueeze(-1)).squeeze(-1) + o_t
...
class PendulumTransition(Transition):
    def __init__(self, z_dim=3, u_dim=1, device='cuda:0'):
        net = nn.Sequential(
            nn.Linear(z_dim, 100),
            nn.BatchNorm1d(100),
            nn.ReLU(),
        ...
        super(PendulumTransition, self).__init__(net, z_dim, u_dim,device)
