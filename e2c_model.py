...
class E2C(nn.Module):
    def __init__(self, obs_dim, z_dim, u_dim, env='planar', device='cuda:0'):
        ...
        self.encoder = enc(obs_dim=obs_dim, z_dim=z_dim).to(self.device)
        # self.encoder.apply(init_weights)
        self.decoder = dec(z_dim=z_dim, obs_dim=obs_dim).to(self.device)
        # self.decoder.apply(init_weights)
        self.trans = trans(z_dim=z_dim, u_dim=u_dim, device=self.device).to(self.device)
